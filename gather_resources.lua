...
button_names = {"Clay","Dirt","Grass","Limestone","Slate","Slate and Clay"};
counter = 0;
scounter = 0;
ccounter = 0;
...
function gatherSlateClay()
	...
	while 1 do
		...
		local clay = srFindImage("clay.png");
			if slate then
			srClickMouseNoMove(slate[0]+5,slate[1]);
			sleepWithStatus(2300, "Clicking Slate Icon\nWaiting on Animation\n\nSlate Collected: " .. tostring(scounter) .. "\n\n\nElapsed Time: " .. getElapsedTime(timeStarted));
			scounter = scounter + 1;
			elseif clay then
			srClickMouseNoMove(clay[0]+5,clay[1]);
			sleepWithStatus(2300, "Clicking Clay Icon\nWaiting on Animation\n\nClay Collected: " .. tostring(ccounter) .. "\n\n\nElapsed Time: " .. getElapsedTime(timeStarted));
			ccounter = ccounter + 1;
			else
			sleepWithStatus(50, "Searching for Slate and Clay Icons\n\nSlate Collected: " .. tostring(scounter) .. "\n\nClay Collected: " .. tostring(ccounter) .. "\n\nElapsed Time: " .. getElapsedTime(timeStarted));
			end
			closePopUp()
	end
end
...
function gatherResources()
	...
	while 1 do
		...
		while not is_done do
			...
			if autoMove then
			 autoMove = CheckBox(35, 200, z, 0x80ff80ff, " Set Automove Direction",
								  autoMove, 0.65, 0.65);
			elseif not autoMove then
			 autoMove = CheckBox(35, 200, z, 0xffffffff, " Set Automove Direction",
								  autoMove, 0.65, 0.65);
			end
			...
			for i=1, #button_names do
				...
				elseif button_names[i] == "Slate" then
					...
					y = 130;
					bsize = 130;
				elseif button_names[i] == "Slate and Clay" then
					x = 25;
					y = 160;
					bsize = 130;
				end
		...
		elseif image_name == "Dirt" then
			gatherDirt();
		elseif image_name == "Slate and Clay" then
			gatherSlateClay();
		end
	end
end
